...
class _ApplicationFormScreenState extends State<ApplicationFormScreen> {
  ...
  void initState() {
    ...
    if (widget.application != null) {
      yearController.text = widget.application!.year;

      module1Controller.text = widget.application!.module1;

      module2Controller.text = widget.application!.module2;

      eligible = widget.application!.eligible;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.application == null ? 'New Application' : 'Edit Application',
                ...
                onPressed: () async {
                  if (formKey.currentState!.validate()) {
                    ...
                    ApplicationModel application = ApplicationModel(
                      ...
                      module2: module2Controller.text,
                      status: widget.application?.status ?? 'Pending',
                      eligible: eligible,
                    );

                    if (widget.application == null) {
                      await applicationVM.addApplication(
                        application,
                      );
                    } else {
                      await applicationVM.updateApplication(
